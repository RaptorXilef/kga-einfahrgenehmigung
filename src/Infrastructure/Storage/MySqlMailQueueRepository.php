...
final readonly class MySqlMailQueueRepository implements MailQueueRepositoryInterface
{
    ...
    public function enqueue(MailJob $job): void
    {
        ...
        $stmt->execute([$job->recipient, $job->subject, $job->template, \json_encode($job->data, \JSON_UNESCAPED_UNICODE), $job->createdAt->format('Y-m-d H:i:s')]);
    }
    ...
    public function processBatch(int $limit, callable $processor): int
    {
        ...
        $this->pdo->exec("UPDATE `{$table}` SET attempts = attempts + 100 WHERE attempts < 3 LIMIT $limit");
        $items = $this->pdo->query("SELECT * FROM `{$table}` WHERE attempts >= 100 ORDER BY created_at ASC")->fetchAll(\PDO::FETCH_ASSOC);
